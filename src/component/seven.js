...
class Seven extends React.Component{
  ...
  async componentDidMount(){
    ...
    this.setState({data:result.slice(0,50), playId:result[6], single:result[6]});
  ...
  render(){
    return(
      <div id="data-container">
         ...
         <div>
           {this.state.showModal ? <Controler values={this.state.mutes} mute={this.Mute} played={this.playIt} isplay={this.state.isplay} data={this.state.data} playing={this.state.single} forward={this.Forward} backward={this.Backward}/> : ''}
         </div>
      </div>
    )
  }
